...
def write_json_schema(path: str | Path, kind: SchemaKind) -> Path:
    """Write the JSON schema to a file and return the resolved path."""

    file_path = Path(path)
    file_path.parent.mkdir(parents=True, exist_ok=True)
    schema = get_json_schema(kind)
    file_path.write_text(json.dumps(schema, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    logger.info("write_json_schema", extra={"path": str(file_path), "kind": kind})
    return file_path
