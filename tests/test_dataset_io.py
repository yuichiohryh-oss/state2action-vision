...
from state2action_vision.dataset.io import read_jsonl, validate_jsonl, write_jsonl
...
def test_validate_jsonl_collects_errors(tmp_path: Path) -> None:
    path = tmp_path / "bad.jsonl"
    path.write_text("{\"image_path\": 123}\nnot json\n", encoding="utf-8")

    report = validate_jsonl(path, parse_dataset_record)
